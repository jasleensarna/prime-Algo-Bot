...
state = {
    ...
    "signal_mode": "lenient",  # lenient | medium | strict
...
# ── SIGNAL MODE CONFIGS ──────────────────────────────────────
# Each mode maps to different indicator params
MODES = {
    "lenient": {
...
async def place_order(symbol, side, order_type, qty=None, usdt_qty=None,
    ...
    if trigger_price:
        body["triggerPrice"] = str(round(trigger_price, 8))
        body["orderType"] = "Limit"
        body["price"] = str(round(trigger_price * 0.99, 8))
        body["triggerDirection"] = 2
        body["orderFilter"] = "StopOrder"
    data = await by_post("/v5/order/create", body)
    if data.get("retCode") != 0:
        raise Exception(data.get("retMsg", f"Order failed: {data.get('retCode')}"))
...
# ── PRIMEFLOW INDICATOR CALCULATIONS ─────────────────────────
...
def calc_supertrend(highs, lows, closes, factor, period):
    """Returns True if bullish on last bar"""
    n = len(closes)
    if n < period + 2: return True

    # Build ATR series using Wilder
    trs = [max(highs[i]-lows[i], abs(highs[i]-closes[i-1]), abs(lows[i]-closes[i-1]))
           for i in range(1, n)]
    atr_s = []
    ...
    # Build supertrend
    direction = 1  # 1=bear, -1=bull
    prev_upper = prev_lower = 0
    last_dir = 1

    for i in range(len(atr_s)):
        ci = i + period  # index into closes
        if ci >= n: break
        hl2 = (highs[ci] + lows[ci]) / 2
        basic_upper = hl2 + factor * atr_s[i]
        basic_lower = hl2 - factor * atr_s[i]

        final_upper = basic_upper if basic_upper < prev_upper or closes[ci-1] > prev_upper else prev_upper
        final_lower = basic_lower if basic_lower > prev_lower or closes[ci-1] < prev_lower else prev_lower

        if closes[ci] > final_upper:
            last_dir = -1  # bullish
        elif closes[ci] < final_lower:
            last_dir = 1   # bearish

        prev_upper = final_upper
        prev_lower = final_lower

    return last_dir < 0  # True = bullish
...
def calc_macd(closes, fast=12, slow=26, signal=9):
    ...
    if len(macd_series) < signal: return macd_series[-1], macd_series[-1]
    sig = calc_ema(macd_series, signal)
    return macd_series[-1], sig

def ema_crossover(values_fast, values_slow, period_fast, period_slow):
    """Returns (cross_up, cross_down) based on last 2 bars"""
    if len(values_fast) < 2: return False, False
    fast_now  = calc_ema(values_fast, period_fast)
    slow_now  = calc_ema(values_slow, period_slow)
    fast_prev = calc_ema(values_fast[:-1], period_fast)
    slow_prev = calc_ema(values_slow[:-1], period_slow)
    cross_up   = fast_prev <= slow_prev and fast_now > slow_now
    cross_down = fast_prev >= slow_prev and fast_now < slow_now
    return cross_up, cross_down

# ── PRIMEFLOW SIGNAL ENGINE ───────────────────────────────────
...
def primeflow_signal(opens, highs, lows, closes, volumes, mode="lenient"):
    ...
    ema_fast  = calc_ema(closes, ef)
    ema_slow  = calc_ema(closes, es)
    ema_trend = calc_ema(closes, et)

    cross_up, cross_down = ema_crossover(closes, closes, ef, es)

    st_bull = calc_supertrend(highs, lows, closes,
                              cfg["st_factor"], cfg["st_period"])
    ...
    # ── LONG conditions ──────────────────────────
    # Lenient: EMA cross + Supertrend + RSI + Trend
    long_base = (cross_up and st_bull and
                 rsi_val < cfg["rsi_ob"] and bull_trend)

    # Medium/Strict add: MACD + Volume + Bull candle
    ...
    # ── SHORT (exit signal for spot longs) ───────
    short_base = (cross_down and not st_bull and
                  rsi_val > cfg["rsi_os"] and bear_trend)
...
# ── SCANNER ───────────────────────────────────────────────────
SKIP = {"BTC","ETH","BNB","SOL","XRP","ADA","DOT","LINK","AVAX","MATIC",
        "UNI","AAVE","LTC","BCH","ETC","ATOM","NEAR","ARB","OP","SUI",
        "TRX","DOGE","SHIB","APT","INJ","TON","FTM","ALGO"}

async def scan_once():
    try:
        instruments = await get_instruments()
        pairs = [i for i in instruments
                 if i.get("quoteCoin") == "USDT"
                 and i.get("status") == "Trading"
                 and i.get("baseCoin","").upper() not in SKIP]
        state["stats"]["scanned"] = len(pairs)

        new_syms = [i.get("symbol","") for i in pairs
                    if i.get("symbol","") not in seen_symbols]
        for s in new_syms: seen_symbols.add(s)
        state["stats"]["new"] = len(seen_symbols)

        # Scan new symbols first
        ...
        # Random sample of known symbols for fresh signals
...
async def analyse(symbol):
    try:
        ...
        # Update signal log for dashboard
        log = {"symbol": symbol, "signal": signal,
               "rsi": details.get("rsi", 0),
               "vol": details.get("vol_ratio", 0),
               "st":  details.get("st_bull", False),
               "cross": details.get("cross_up", False),
...
async def execute_buy(symbol, usdt_amount, atr_val=0, entry_price=0):
    ...
    await place_order(symbol, "Buy", "Market", usdt_qty=usdt_amount)
    ...
    # Use ATR-based TP/SL if available, else percentage fallback
    if atr_val > 0:
        tp1 = round(price + atr_val * state["tp_mult1"], 8)
        tp2 = round(price + atr_val * state["tp_mult2"], 8)
        sl  = round(price - atr_val * state["sl_mult"],  8)
    else:
        tp1 = round(price * 1.30, 8)
        tp2 = round(price * 1.70, 8)
        sl  = round(price * 0.85, 8)

    half = round(qty / 2, 6)
    await place_order(symbol, "Sell", "Limit",  qty=half, price=tp1)
    await place_order(symbol, "Sell", "Limit",  qty=half, price=tp2)
    await place_order(symbol, "Sell", "Market", qty=qty, trigger_price=sl)

    state["positions"][symbol] = {
        "symbol": symbol, "entry": price, "current": price,
        "qty": qty, "usdt_size": usdt_amount,
        "tp1": tp1, "tp2": tp2, "sl": sl,
        "tp1_hit": False, "pnl": 0.0, "pnl_pct": 0.0,
        "mode": state["signal_mode"],
        "entry_ts": int(time.time()),
    }
    state["stats"]["traded"] += 1
    state["balance"] = await get_balance()
    print(f"BOUGHT {symbol} @ {price} | TP1={tp1} TP2={tp2} SL={sl}")
    ...
    for sym in list(state["positions"]):
        try:
            ...
            if not pos["tp1_hit"] and price >= pos["tp1"]:
                ...
                pos["qty"] /= 2
                state["today_pnl"] += pos["pnl"] * 0.5
                state["total_pnl"] += pos["pnl"] * 0.5
                print(f"TP1 HIT: {sym} @ {price}")

            if price >= pos["tp2"]:
                state["today_pnl"] += pos["pnl"]; state["total_pnl"] += pos["pnl"]
                state["wins"] += 1
                state["trades"].insert(0, {**pos, "result":"win","close_ts":int(time.time())})
                del state["positions"][sym]; state["balance"] = await get_balance()
                print(f"TP2 HIT: {sym} WIN"); continue
...
async def connect(req: Request):
    ...
    state["risk_pct"]    = float(b.get("risk_pct", 5.0))
    state["max_pos"]     = int(b.get("max_pos", 2))
...
async def set_mode(mode: str):
    ...
    print(f"Signal mode changed to: {mode}")
...
async def tradingview_webhook(req: Request):
    try:
        ...
        if not symbol or not action:
            return JSONResponse({"error":"Missing symbol or action"}, 400)
        if action == "buy":
            ...
            usdt = max(state["balance"] * state["risk_pct"] / 100, 6.0)
            await execute_buy(symbol, usdt)
            return {"ok":True,"msg":f"BUY: {symbol}"}
        if action == "sell":
