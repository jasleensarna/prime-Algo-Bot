...
pos.get(“entry”,0), exit_price, pos.get(“pnl”,0), pos.get(“pnl_pct”,0),
pos.get(“usdt_size”,0), pos.get(“leverage”,2),
pos.get(“tp1_pct”,0), pos.get(“tp2_pct”,0), pos.get(“tp3_pct”,0), pos.get(“sl_pct”,0),
pos.get(“score”,0), pos.get(“label”,””), pos.get(“mode”,“lead”),
bool(pos.get(“tp1_hit”,False)), bool(pos.get(“tp2_hit”,False)), int(time.time()))
...
await conn.execute(”””
UPDATE bot_stats SET wins=wins+$1,losses=losses+$2,total_pnl=total_pnl+$3,
long_count=long_count+$4,short_count=short_count+$5,traded=traded+1,updated_at=NOW()
...
try:
async with _db_pool.acquire() as conn:
rows = await conn.fetch(“SELECT * FROM trades ORDER BY close_ts DESC LIMIT $1”, limit)
...
h = {“X-BAPI-API-KEY”:state[“api_key”],“X-BAPI-TIMESTAMP”:ts,“X-BAPI-RECV-WINDOW”:“5000”}
if signed: h[“X-BAPI-SIGN”] = by_sign(state[“api_secret”], ts, qs)
async with httpx.AsyncClient(timeout=15) as c:
r = await c.get(f”{BYBIT}{path}”, params=p, headers=h); return r.json()

async def by_post(path, body, signed=True):
ts = str(int(time.time()*1000)); bs = json.dumps(body)
h = {“X-BAPI-API-KEY”:state[“api_key”],“X-BAPI-TIMESTAMP”:ts,
“X-BAPI-RECV-WINDOW”:“5000”,“Content-Type”:“application/json”}
if signed: h[“X-BAPI-SIGN”] = by_sign(state[“api_secret”], ts, bs)
async with httpx.AsyncClient(timeout=15) as c:
r = await c.post(f”{BYBIT}{path}”, content=bs, headers=h); return r.json()

async def get_balance():
try:
d = await by_get(”/v5/account/wallet-balance”,{“accountType”:“UNIFIED”},signed=True)
if d.get(“retCode”)==0:
for w in d.get(“result”,{}).get(“list”,[]):
for c in w.get(“coin”,[]):
if c.get(“coin”)==“USDT”:
b = float(c.get(“walletBalance”) or c.get(“equity”) or 0)
if b>0: return b
except: pass
return 0.0

async def get_price(symbol):
d = await by_get(”/v5/market/tickers”,{“category”:“linear”,“symbol”:symbol})
...
d = await by_post(”/v5/order/create”, body)
if d.get(“retCode”)!=0: print(f”Close failed {symbol}: {d.get(‘retMsg’)}”)
return d.get(“result”,{})

async def cancel_all_orders(symbol):
try: await by_post(”/v5/order/cancel-all”,{“category”:“linear”,“symbol”:symbol})
except: pass
...
r = d.get(“result”,{})
bids = r.get(“b”,[]); asks = r.get(“a”,[])
if not bids or not asks: return {“obi”:0.5,“signal”:“neutral”}
...
bot5a = sum(float(a[1]) for a in asks[-5:]) if len(asks)>=10 else top5a
...
return {“obi”:round(obi,3),“signal”:signal,“bid_vol”:round(bid_vol,2),
“ask_vol”:round(ask_vol,2),“bid_stack”:bid_stack,“ask_stack”:ask_stack}
except Exception as e:
...
if not trades: return {“buy_ratio”:0.5,“signal”:“neutral”,“trade_count”:0}
...
if buy_ratio > 0.60 or (buy_ratio > 0.53 and accel_buy):   signal = “long”
elif buy_ratio < 0.40 or (buy_ratio < 0.47 and accel_sell): signal = “short”
else:                                                         signal = “neutral”
return {“buy_ratio”:round(buy_ratio,3),“signal”:signal,
“trade_count”:len(trades),“accel_buy”:accel_buy,“accel_sell”:accel_sell}
except Exception as e:
...
hd = await by_get(”/v5/market/funding/history”,{“category”:“linear”,“symbol”:symbol,“limit”:10})
...
return {“rate”:round(current,4),“avg”:round(avg,4),“momentum”:round(momentum,4),“signal”:signal}
except Exception as e:
...
print(f”Lead signal error {symbol}: {e}”); return None, {}
...
elif score >= 65: tp1_pct,tp2_pct,tp3_pct,sl_pct =  7.0,18.0,30.0,3.5;  label=“STRONG”
elif score >= 40: tp1_pct,tp2_pct,tp3_pct,sl_pct =  5.0,12.0,20.0,3.0;  label=“MEDIUM”
else:             tp1_pct,tp2_pct,tp3_pct,sl_pct =  3.0, 7.0,12.0,2.0;  label=“WEAK”
if signal==“long”:
tp1=round(entry*(1+tp1_pct/100),6); tp2=round(entry*(1+tp2_pct/100),6)
tp3=round(entry*(1+tp3_pct/100),6); sl =round(entry*(1-sl_pct /100),6)
else:
tp1=round(entry*(1-tp1_pct/100),6); tp2=round(entry*(1-tp2_pct/100),6)
tp3=round(entry*(1-tp3_pct/100),6); sl =round(entry*(1+sl_pct /100),6)
...
try:
d = await by_get(”/v5/market/open-interest”,{“category”:“linear”,“symbol”:symbol,“intervalTime”:“5min”,“limit”:10})
items = d.get(“result”,{}).get(“list”,[])
...
try:
d = await by_get(”/v5/market/liquidation”,{“category”:“linear”,“symbol”:symbol,“limit”:50})
items = d.get(“result”,{}).get(“list”,[])
if not items: return “neutral”
lv = sum(float(i.get(“size”,0))*float(i.get(“price”,0)) for i in items if i.get(“side”)==“Buy”)
sv = sum(float(i.get(“size”,0))*float(i.get(“price”,0)) for i in items if i.get(“side”)==“Sell”)
t  = lv+sv
...
try:
r = await asyncio.gather(get_oi_trend(symbol), get_liquidation_bias(symbol), return_exceptions=True)
...
if all_syms: TOP50.clear(); TOP50.extend(all_syms)
sample = random.sample(TOP50, min(20, len(TOP50)))
state[“stats”][“scanned”] = len(TOP50)
print(f”Scan: {len(sample)} of {len(TOP50)} | positions={len(state[‘positions’])}”)
for sym in sample:
...
usdt  = max(get_dynamic_position_size(score, state[“balance”]), MIN_ORDER)
await execute_trade(sig[“symbol”], sig[“direction”], usdt, sig)
...
direction, details = await lead_signal(symbol)
log = {“symbol”:symbol,“signal”:direction,
“obi”:details.get(“obi”,0.5),“buy_ratio”:details.get(“buy_ratio”,0.5),
“funding”:details.get(“funding_rate”,0),“ts”:int(time.time())}
...
“score”:score,“label”:label,“est_size”:est_size,
“obi”:details.get(“obi”,0.5),“buy_ratio”:details.get(“buy_ratio”,0.5),
“funding”:details.get(“funding_rate”,0),
“oi”:ctx.get(“oi_trend”,“neutral”),“liq”:ctx.get(“liq_bias”,“neutral”),
...
await set_leverage(symbol); await asyncio.sleep(0.3)
price = await get_price(symbol)
...
“qty”:qty,“qty_remaining”:qty,“usdt_size”:usdt_amount,
...
“pnl”:0.0,“pnl_pct”:0.0,
“mode”:“lead”,“leverage”:state[“leverage”],“entry_ts”:int(time.time()),
}
...
```
                if direction=="long":
                    ...
                    if sym in state["positions"] and price <= pos["sl"]:
                        ...
                        await close_position_and_record(sym,"loss"); continue
...
state[“today_pnl”] += pnl; state[“total_pnl”] += pnl
...
if saved: state[“trades”] = saved; print(f”Loaded {len(saved)} trades from DB”)
...
if syms: TOP50.clear(); TOP50.extend(syms); print(f”Loaded {len(TOP50)} symbols”)
...
dlp = round((state[“start_balance”]-state[“balance”])/state[“start_balance”]*100,2)
return {
...
“bot_on”:         state[“bot_on”],
“signal_mode”:    “lead”,
“mode_desc”:      “Order Book + Trade Flow + Funding Momentum”,
...
return {“ok”:True,“mode”:“lead”,“desc”:“Order Book + Trade Flow + Funding Momentum”}
...
state[“daily_loss_hit”] = False; state[“bot_on”] = True
state[“start_balance”]  = state[“balance”]; state[“today_pnl”] = 0.0
return {“ok”:True}
